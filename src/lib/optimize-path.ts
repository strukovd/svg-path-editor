...
const toStr = (pt: SimplePoint) => {
  return [String(pt.x), String(pt.y)];
};

const optimizeRelativeAbsolute = (svg: Path) => {
  let length = svg.asString(4, true).length;
  const o = new SimplePoint(0,0);
  for(let i=0 ; i<svg.d.length; ++i) {
    const previous = i>0 ? svg.d[i-1] : null;
    const comp = svg.d[i];
...
export const optimizePath = (svg: Path, {
  ...
  const path = svg.d;
  const o = new SimplePoint(0,0);
  let initialPt = new SimplePoint(0,0);
  for(let i=1 ; i<path.length ; ++i) {
    ...
    if(useHorizontalAndVerticalLines) {
      if(c1type === 'L') {
        ...
        if(tg.x === c1.previousPoint.x) {
          path[i] = PathCommand.MakeFrom(c1, c0, 'V');
          continue;
        }
        if(tg.y === c1.previousPoint.y) {
          path[i] = PathCommand.MakeFrom(c1, c0, 'H');
          continue;
        }
      }
    }
    if(useShorthands) {
      if((c0type === 'Q' || c0type === 'T') && c1type === 'Q') {
        ...
        const candidate = PathCommand.Make(['T', ...pt]);
      ...
      if((c0type === 'C' || c0type === 'S') && c1type === 'C') {
        ...
        const candidate = PathCommand.Make(['S', ...ctrl, ...pt]);
        candidate.refresh(o, c0);
      ...
      if((c0type !== 'C' && c0type !== 'S') && c1type === 'C') {
        if(c1.previousPoint.x === c1.absolutePoints[0].x && c1.previousPoint.y === c1.absolutePoints[0].y) {
          ...
          path[i] = PathCommand.Make(['S', ...ctrl, ...pt]);
          path[i].refresh(o, c0);
        }
      }
