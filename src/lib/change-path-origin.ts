import { Path, PathCommand } from "./svg";
import { optimizePath } from "./optimize-path";
import { getSubPathBounds } from "./get-sub-path-bounds";
import type { PathCommands } from "./svg-command-types";

export const changePathOrigin = (svg: Path, newOriginIndex: number, subpath?: boolean)=> {
  if(svg.d.length <= newOriginIndex || newOriginIndex === 0) {
    return;
  }
  ...
  const newFirstItem = svg.d[newOriginIndex];
  const newLastItem = svg.d[newOriginIndex - 1];
  const firstItemType = newFirstItem.getType().toUpperCase();
  // Shorthands must be converted to be used as origin
  if (firstItemType === 'S') {
    svg.changeType(newFirstItem, (newFirstItem.relative ? 'c' : 'C') as PathCommands);
  } else if (firstItemType === 'T') {
    svg.changeType(newFirstItem, (newFirstItem.relative ? 'q' : 'Q') as PathCommands);
  }
  ...
  for(let i=0 ; i<l ; ++i) {
    if(i===0) {
      ...
      outputPath.push(PathCommand.Make(['M', String(newOrigin.x), String(newOrigin.y)]));
    }
